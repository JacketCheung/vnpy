...
class ZeroStrategy(CtaTemplate):
    ...
    def buyTickCelve(self,tick):
        ...
        if tick.datetime.second > 51:
            ...
            if self.posdetail.longPos > 0 and am.macd > 0 and  am.lastmacd > 0 and self.bg.bar.open < tick.lastPrice and not self.oneMinute:
                fangxiang = duoping
                price = tick.bidPrice1
                self.chulikaipingcang(fangxiang,price)
        ...
        if tick.datetime.second > 51:
            if am.diff < 0 and am.macd > 0 and am.lastmacd < 0:
                ...
                price = tick.askPrice1
                    ...
                    price = tick.bidPrice1
            self.chulikaipingcang(fangxiang,price)
        if self.posdetail.shortPos > 0 and am.macd < 0 and am.lastmacd < 0 and self.bg.bar.open > tick.lastPrice and not  self.oneMinute:
            fangxiang = kongping
            price = tick.askPrice1
            self.chulikaipingcang(fangxiang,price)
    ...
    def onBar(self, bar):
        ...
        if  not self.didinited:
            ...
            print('detial', posdetial.shortPos, posdetial.shortTd, posdetial.shortTdFrozen,posdetial.shortTd - posdetial.shortTdFrozen)
            print('longdetial', posdetial.longPos, posdetial.longTd, posdetial.longTdFrozen,posdetial.longTd - posdetial.longTdFrozen)
    ...
    def onOrder(self, order):
        ...
        if order.status == STATUS_CANCELLED and self.genOrder is not None:
            ...
            self.writeCtaLog(int(self.genOrder))
...
#----------------------------------------------------------------------
    ...
    def isTradeContinueTime(self, firstdatetime, seconddatetime):
        ...
        if self.iscontinueTime(firstdatetime, seconddatetime):
            ...
            return True
                ...
                firstdatetime.minute == 29 or firstdatetime.minute == 30) and seconddatetime.hour == 13 and (seconddatetime.minute == 30 or seconddatetime.minute == 31 or seconddatetime.minute == 29):
            return True
    ...
    def checkCancelOrder(self):
        if self.buyOrder :
            if self.oneMinute and self.am.macd > 0 :
